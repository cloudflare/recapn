...
impl UncheckedNode {
    pub fn new(node: &ReaderOf<Node>) -> Result<Self> {
...
impl SchemaLoader {
    ...
    pub fn load(&mut self, node: &ReaderOf<Node>) -> Result<()> {
        let id = node.id();
    ...
    pub fn get(&self, id: Id) -> Option<ReaderOf<Node>> {
    ...
    pub fn resolve_brand<'a>(&'a self, brand: &ReaderOf<'a, Brand>) -> Result<Branding> {
