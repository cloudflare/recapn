...
macro_rules! get_cmp {
    ($ty1:ty, $ty2:ty) => {
        ...
        impl PartialEq<&'_ $ty1> for $ty2 {
            fn eq(&self, other: &&'_ $ty1) -> bool {
                self.get().eq(&other.get())
            }
        }

        impl PartialOrd<&'_ $ty1> for $ty2 {
            fn partial_cmp(&self, other: &&'_ $ty1) -> Option<cmp::Ordering> {
                self.get().partial_cmp(&other.get())
            }
        }
    };
...
impl NonZeroU29 {
    ...
    /// Adds two segment sizes together, saturating at the numeric bounds instead of overflowing.
    #[inline]
    pub fn saturating_add(self, n: Self) -> Self {
        let new_value = cmp::min(Self::MAX_VALUE, self.get().saturating_add(n.get()));
        Self(NonZeroU32::new(new_value).unwrap())
    }
}

pub type AllocLen = NonZeroU29;
pub type SegmentLen = NonZeroU29;
...
impl Segment {
    #[inline]
    pub const unsafe fn new(data: NonNull<Word>, len: SegmentLen) -> Self {
        Self { data, len }
    }
    ...
    pub fn as_slice(&self) -> &[Word] {
        unsafe {
            std::slice::from_raw_parts(self.data.as_ptr() as *const _, self.len.get() as usize)
        }
    }
    ...
    pub fn as_mut_slice(&mut self) -> &mut [Word] {
        unsafe {
            std::slice::from_raw_parts_mut(self.data.as_ptr() as *mut _, self.len.get() as usize)
        }
    }

    #[inline]
    pub fn as_bytes(&self) -> &[u8] {
        unsafe { std::slice::from_raw_parts(self.data.as_ptr() as *const _, self.len_as_bytes()) }
    }

    #[inline]
    pub fn as_mut_bytes(&mut self) -> &mut [u8] {
        unsafe { std::slice::from_raw_parts_mut(self.data.as_ptr() as *mut _, self.len_as_bytes()) }
    }
}
...
pub unsafe trait Alloc {
    ...
    /// [`Word`]: Word
    /// [`Segment`]: Segment
    unsafe fn alloc(&mut self, size: SegmentLen) -> Segment;
...
impl<A> Fixed<A> {
    #[inline]
    pub const fn new(size: SegmentLen, inner: A) -> Self {
        Self { size, inner }
    }
}
...
pub struct Growing<A> {
    next_segment: SegmentLen,
    inner: A,
}

impl<A> Growing<A> {
    /// The default length used for the first segment of 1024 words.
    pub const DEFAULT_FIRST_SEGMENT_LEN: SegmentLen = SegmentLen::new(1024).unwrap();

    #[inline]
    pub const fn new(first_segment: SegmentLen, alloc: A) -> Self {
        Self {
...
unsafe impl<A: Alloc> Alloc for Growing<A> {
    ...
    unsafe fn alloc(&mut self, size: SegmentLen) -> Segment {
        ...
        self.next_segment = self.next_segment.saturating_add(alloc_size);
...
impl<'s, A> Scratch<'s, A> {
    /// Creates a new scratch space allocator.
    #[inline]
    pub fn with_space<const N: usize>(space: &'s mut Space<N>, alloc: A) -> Self {
        let size = SegmentLen::new(N as u32).unwrap();
        let start = NonNull::new(space.0.as_mut_ptr()).unwrap();

        Self {
            s: PhantomData,
            // SAFETY: we know this is a valid memory segment
            segment: unsafe { Segment::new(start, size) },
            used: false,
            next: alloc,
        }
    }
    ...
    pub fn with_dyn_space(space: &'s mut DynSpace, alloc: A) -> Self {
        let size = SegmentLen::new(space.0.len() as u32).unwrap();
        let start = NonNull::new(space.0.as_mut_ptr()).unwrap();
...
mod tests {
    ...
    macro_rules! alloc {
        ($alloc:expr, $size:expr, $func:expr) => {{
            let alloc = &mut $alloc;
            unsafe {
                let segment = alloc.alloc(SegmentLen::new($size).unwrap());
                $func(segment.clone());
    ...
    #[test]
    fn fixed_alloc() {
        let mut fixed = Fixed::new(SegmentLen::new(5).unwrap(), Global);
    ...
    #[test]
    fn growing_alloc() {
        let mut growing = Growing::new(SegmentLen::MIN, Global);
    ...
    #[test]
    fn scratch_alloc() {
        let mut static_space = Space::<5>::VALUE;
        let mut addr = 0;
        {
